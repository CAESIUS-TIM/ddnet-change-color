    ...
    try:
        ...
        for plugin in required_plugins:
            ...
            if plugin_dir.exists():
                plugins.append((str(plugin_dir), f"PySide6/Qt/plugins/{plugin}"))
...
def collect_translations() -> List[Tuple[str, str]]:
    ...
    try:
        ...
        for file in qt_translations_path.glob("*.qm"):
            translations.append((str(file), f"PySide6/Qt/translations/{file.name}"))
...
def validate_assets():
    ...
    if "icon" in config:
        ...
        if not icon_path.exists():
            ...
            print("      使用 generate_icons.py 生成图标")
...
def collect_data_files() -> List[Tuple[str, str]]:
    ...
    if "icon" in config:
        ...
        if icon_path.exists():
            # 图标文件复制到应用根目录
            data_files.append((str(icon_path), icon_path.name))
